...
<Steps>
  ...
  <Step title="Enable browser notifications">
    ...
    You'll then get system-level approval requests — and security alerts — in this browser even when the Sesame tab is closed; clicking one jumps straight to the approval screen. (Approvals also go to Telegram and the Sesame app, so you can use whichever channel you prefer.)

    <Tip>
      Enable this **before** your first request to a new hostname, so the very first approval prompt reaches you instead of the request sitting idle until it times out.
    </Tip>
  </Step>
</Steps>
