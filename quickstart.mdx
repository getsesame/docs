...
Sesame keeps your API credentials out of agent prompts by injecting them server-side at the broker. This guide walks you from a fresh machine to a working brokered request in under five minutes.

## Prerequisites

- **Operating system:** macOS (arm64 or x86_64) or Linux (x86_64)
- **Sesame account:** sign up or sign in at [getsesame.dev](https://getsesame.dev)
- **Optional:** Node.js 18+ if you want to install agent skills automatically
...
<Steps>
  <Step title="Install the CLI">
    ...
    <Tip>
      If Node.js is available on your machine, the installer also runs
      `npx skills add getsesame/skills --yes --global --all` automatically,
      so your agent skills are ready the moment the CLI is installed.
    </Tip>
  </Step>
  ...
  <Step title="Install agent skills (optional)">
    Agent skills teach supported AI agents (Claude Code, Codex, Cursor, OpenClaw, and 40+ others) to route their API calls through Sesame automatically:

    ```bash
    npx skills add getsesame/skills
    ```

    After installation, agents that previously called APIs directly will use `sesame request` instead — no prompt engineering or manual tool wiring required.
  </Step>
</Steps>
