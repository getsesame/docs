...
<Steps>
  ...
  <Step title="Make an authenticated request">
    ...
    <Warning>
      If the host you want isn't in your `sesame hostnames` list, configure its secret first — then come back to this step. You can do this two ways:

      - **CLI:** `sesame secret create <name> --hostname <host>` returns a dashboard link where you paste the value (the CLI never accepts secret material).
      - **Dashboard:** open **Credentials → Secrets** and add it there directly.

      Don't assume a host is usable just because the API exists.
    </Warning>
