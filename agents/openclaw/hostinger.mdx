...
Requires **sesame ≥ v0.3.71** (`sesame --version`) — earlier builds leak `LD_LIBRARY_PATH` into the wrapped agent and break any child that links system libraries (e.g. `curl: … libssl.so.3: version 'OPENSSL_3.2.0' not found`). `install.sh` pulls the latest, so a fresh install is fine.

Two setup lines, both needed on a **fresh** template:

* The `.config`/`.local` chown fixes a fresh-template quirk — those dirs are created **root-owned** on the `/data` mount, so `sesame login` (run as `node`) crashes with `PermissionError: '/data/.config/sesame'`. (Once fixed they stay fixed: `/data` survives recreates.)
* The runtime-dir line creates `/run/user/1000` — containers have no systemd/logind to make it, and `node` can't (`/run` is root-owned) — without it `login` crashes with `PermissionError: '/run/user/1000'`.
...
<Warning>
  ...
  Both proxy and CA variables present → the gateway was born under `sesame launch`. On **sesame ≥ v0.3.71** `LD_LIBRARY_PATH` should be **absent** from that output — if it's set to a `…/lib/sesame` path, you're on a pre-fix build that leaks it into the agent (upgrade).
</Warning>
...
| Error | Cause → Fix |
| --- | --- |
| `PermissionError: [Errno 13] Permission denied: '/data/.config/sesame'` | `/data/.config` is created **root-owned** on a fresh template, so `sesame login` (run as `node`) can't write it. [Step 4](#4-install-the-sesame-cli) now chowns it up front — if you still hit this, run `mkdir -p /data/.config /data/.local && chown -R node:node /data/.config /data/.local`, then retry `login`. |
...
| Wrapped chat returns `{"detail":"Bad Request"}` (model call fails after a ~40s stall) | A **brokered host is replacing the agent's own working auth.** If OpenClaw routes its LLM through an OAuth host you've brokered — e.g. `chatgpt.com` via the codex path — the broker swaps the agent's valid token for your stored credential and the provider rejects it. (The broker logs `200` — that's its envelope; the real `400` rides *inside* from upstream, which is why it isn't a Sesame error.) Fix: broker **`api.openai.com`** (Bearer) and run a standard `provider: openai` model, **not** the codex/`chatgpt.com` path. Don't broker the OAuth host. |
